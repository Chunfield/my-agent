...
export default function ChatApp({ forceLoggedIn, forceLoggedOut }: ChatAppProps) {
  const { data: session } = useSession();

  const effectiveSession = (() => {
    if (forceLoggedIn) {
      return { user: { name: '预览用户 (已登录)', image: '/api/placeholder/32/32' } };
    }
    if (forceLoggedOut) {
      return { user: null };
    }
          ...
          <div className="flex items-center gap-3">
            {effectiveSession?.user ? (
              <div className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-full bg-gray-100 text-gray-600">
                ...
                />
                <span className="hidden sm:inline">{effectiveSession.user.name}</span>
              </div>
            ) : (
              <button
          ...
          <div className="max-w-3xl mx-auto">
            ...
            >
              <textarea
                ...
                placeholder={
                  effectiveSession?.user ? '发送消息给 DeepSeek...' : '请先登录'
                }
                disabled={!effectiveSession?.user}
              ...
              <button
                ...
                disabled={(!input.trim() && !isLoading) || !effectiveSession?.user}
                className={cn(
                  'p-2.5 rounded-xl flex-shrink-0 transition-all mb-1',
                  (input.trim() || isLoading) && effectiveSession?.user
